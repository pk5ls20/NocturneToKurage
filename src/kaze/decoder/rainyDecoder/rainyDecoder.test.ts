import { expect, test } from 'vitest';
import { rainyDecoder } from '@/kaze/decoder/rainyDecoder/rainyDecoder';
import { parseHexStrToBuffer } from '@/kaze/decoder/utils';

test('rainyDecodeProto1', () => {
  const decoded = new rainyDecoder(false).decode(
    parseHexStrToBuffer(
      '0a 2b 0a 04 08 7b 10 0a 12 1f a8 06 01 b0 06 02 c0 0c 03 ca 0c 0d 08 01 12 09 e5 93 a6 e5 93 a6 e5 93 a6 d2 0c 03 08 d7 08 1a 02 08 01 12 53 0a 3d 0a 39 08 80 08 12 0f e7 8e a9 e5 8e 9f e7 a5 9e e7 8e a9 e7 9a 84 1a 04 73 68 61 31 22 09 66 69 6c 65 41 2e 6a 70 67 2a 08 08 01 10 01 18 00 20 00 30 80 0f 38 b8 08 40 64 48 01 10 01 10 01 18 00 20 b1 d1 f9 d6 03 28 01 32 02 50 0a 38 64 40 00'
    )
  );
  console.log('decodeProto1', JSON.stringify(decoded));
  expect(decoded.fields.length).toEqual(2);
});

test('rainyDecodeProto2', () => {
  // ApplyUploadRespV3
  const decoded = new rainyDecoder(true).decode(
    parseHexStrToBuffer(
      ''
    )
  );
  console.log('decodeProto2', JSON.stringify(decoded));
  expect(decoded).toEqual({
    '10': '114514',
    '20': 'simpleTest',
    '30': '192993',
    '40': '113130',
    '50': '13131',
    '60': '114.114.114.114',
    '70': 'google.com',
    '80': '22',
    '90': '67b996e7-47a9-4f32-84e5-77c2c33feab3',
    '100': 'R',
    '110': '0',
    '120': '0',
    '130': ['1.1.1.1', '2.2.2.2'],
    '140': '0',
    '150': 'github.com',
    '160': 'github.com',
    '170': '8.8.8.8',
    '200': '原来你也玩原神？',
    '220': '',
  });
});

test('rainyDecodeProto3', () => {
  // NTV2RichMediaReq, with repeated fields
  const decoded = new rainyDecoder(true).decode(
    parseHexStrToBuffer(
      ''
    )
  );
  console.log('decodeProto3', JSON.stringify(decoded));
  expect(decoded).toEqual({
    '1': {
      '1': { '1': '123', '2': '10' },
      '2': { '101': '1', '102': '2', '200': '3', '201': { '1': '1', '2': '哦哦哦' }, '202': { '1': '1111' } },
      '3': { '1': '1' },
    },
    '2': {
      '1': {
        '1': {
          '1': '1024',
          '2': '玩原神玩的',
          '3': 'sha1',
          '4': 'fileA.jpg',
          '5': { '1': '1', '2': '1', '3': '0', '4': '0' },
          '6': '1920',
          '7': '1080',
          '8': '100',
          '9': '1',
        },
        '2': '1',
      },
      '2': '1',
      '3': '0',
      '4': '987654321',
      '5': '1',
      '6': { '10': '10' },
      '7': '100',
      '8': '0',
    },
  });
});
