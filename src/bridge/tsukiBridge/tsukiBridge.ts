...
export class tsukiBridge<EPAT = null> extends dataBridge<number | string, string, extendDataType, EPAT> {
  ...
  bake(data: [Buffer, EPAT?], decoder: protobufDecoder<true, number | string, string, never>): [ProtoTree<number | string, string>, extendDataType] {
    const [bufferData, extraArgs] = data;
    const [rawData, ed] = this.pasteData(bufferData)
    const decodedData = Array.isArray(extraArgs) ?
      decoder.decode(rawData, ...extraArgs) :
